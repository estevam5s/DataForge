...
// A fonte e 'site/scripts/conteudo/tipos_nomeados.py' — mude la e rode o gerador.
...
const blocos: Bloco[] = [
  ...
  {"table": {"head": ["Forma", "Escreve-se", "Serve para"], "rows": [["alias", "`type Id := Integer`", "um nome para o mesmo tipo, em 200 arquivos"], ["alias genérico", "`type Par<T> := Cluster<T>`", "uma função de tipo: o nome recebe o parâmetro"], ["união", "`type Json := String | Integer | Void`", "o valor é um destes, e nada mais"], ["interseção", "`type Auditavel := Serial & Ordenavel`", "tem de ser os dois ao mesmo tempo"], ["refinamento", "`type Positivo := Integer where valor bigger 0`", "o tipo com uma regra, cobrada em toda fronteira"], ["opaco", "`opaque type Cpf := String where len(valor) is 11`", "só nasce validado, e um texto não serve no lugar"]]}},
...
assert len(C.set([a, b])) is 1                 // hash pelo valor`, lang: 'df' },
  ...
  {"table": {"head": ["Código", "Acusa"], "rows": [["`tipo-refinado`", "o literal quebra a regra do `where`"], ["`tipo-uniao`", "o literal não é nenhum dos membros da união"], ["`tipo-intersecao`", "o valor não é todas as partes"], ["`tipo-opaco`", "um valor cru onde o tipo é opaco, ou `Cpf(…)` com a base errada"], ["`tipo-circular`", "`type A := B` e `type B := A`"], ["`declaracao-repetida`", "dois `type` com o mesmo nome no arquivo"], ["`unknown-type`", "um nome que não existe, com sugestão"]]}},
...
assert c.valor is "12345678901" `, lang: 'df', title: `main.df` },
  ...
  {"table": {"head": ["Não existe", "Por quê"], "rows": [["apagamento de tipo em tempo de compilação", "o DataForge interpreta a árvore: a anotação vira conferência em execução, e não some"], ["garantia de ABI ou compatibilidade binária", "não há binário — isso é assunto de linguagem compilada com layout fixo"], ["prova formal de que a regra nunca falha", "o `where` é **verificado**, não provado: o analisador decide sobre literais, e o resto é conferido quando roda"], ["variância declarada (`in`/`out` em `<T>`)", "ainda não; `Cluster<T>` é conferido item a item na fronteira"]]}},
...
const headings = [{ id: 'alias-um-nome-o-mesmo-tipo', text: "Alias: um nome, o mesmo tipo", level: 2 as const }, { id: 'alias-generico-a-funcao-de-tipo', text: "Alias genérico: a função de tipo", level: 3 as const }, { id: 'uniao-um-destes-e-nada-mais', text: "União: um destes, e nada mais", level: 2 as const }, { id: 'intersecao-os-dois-ao-mesmo-tempo', text: "Interseção: os dois ao mesmo tempo", level: 2 as const }, { id: 'refinamento-o-tipo-com-uma-regra', text: "Refinamento: o tipo com uma regra", level: 2 as const }, { id: 'opaco-o-tipo-que-so-nasce-validado', text: "Opaco: o tipo que só nasce validado", level: 2 as const }, { id: 'o-que-o-analisador-prova-antes-de-rodar', text: "O que o analisador prova antes de rodar", level: 2 as const }, { id: 'entre-arquivos', text: "Entre arquivos", level: 2 as const }, { id: 'o-que-isto-nao-e', text: "O que isto não é", level: 2 as const }];
