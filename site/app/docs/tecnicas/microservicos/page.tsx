...
// A fonte e 'site/scripts/conteudo/microservicos.py' — mude la e rode o gerador.
...
const blocos: Bloco[] = [
  ...
  {"table": {"head": ["O que se faz", "O que acontece"], "rows": [["tentar de novo, sem cuidado", "o pedido é processado duas vezes"], ["esperar sem prazo", "uma thread presa por serviço, para sempre"], ["insistir num serviço caído", "ele nunca se recupera, porque nunca para de receber"], ["propagar só o dado", "o rastro se perde na primeira fronteira"]]}},
...
status    0  →  NÃO CHEGOU (ou a resposta se perdeu)`, lang: 'text' },
  ...
  {"table": {"head": ["Situação", "Repete?"], "rows": [["`GET`, `HEAD`, `OPTIONS`, `PUT`, `DELETE`", "sim — não mudam estado, ou são idempotentes"], ["`POST` ou `PATCH` **sem** chave", "**não** — um pagamento repetido cobra duas vezes"], ["`POST` **com** `Idempotency-Key`", "sim — é o servidor que garante o efeito único"], ["5xx, 429, 408, 425", "sim — o problema é do outro lado"], ["outro 4xx", "**não** — o pedido está errado; repetir dá o mesmo erro com mais latência"], ["`status: 0`", "sim — não chegou, vale tentar"]]}},
...
server pedidos on 8080:
  ...
  {"table": {"head": ["Chamada", "Faz"], "rows": [["`Malha.propagar(req, origem)`", "continua o rastro que chegou, ou começa um"], ["`Malha.rastro()`", "o id de agora — para pôr no log"], ["`Malha.contexto()`", "o vault inteiro: rastro, origem, extra"], ["`Malha.comecar_contexto(id, origem, extra)`", "à mão, fora de uma rota"], ["`Malha.cabecalhos_de_contexto()`", "o que vai nas chamadas de saída"]]}},
...
ignite pedidos on 8080 at "0.0.0.0"`, lang: 'df' },
  ...
  {"table": {"head": ["", "O quê"], "rows": [["1", "o estoque fora do ar devolve **503**, e não uma exceção que vira 500 — a diferença muda o que o balanceador faz"], ["2", "a reserva leva `chave := \"pedido-N\"`: se a resposta se perder e o cliente repetir, o estoque não baixa duas vezes"], ["3", "o pedido só fica `reservado` se a reserva **confirmou** — a transação desfaz o resto"], ["4", "o `rastro` volta na resposta de erro: quem recebeu o 502 pode dizer qual foi, e o log dos três serviços é pesquisável por ele"]]}},
...
otherwise:
  ...
  {"table": {"head": ["", "O quê", "Por quê"], "rows": [["1", "a compensação roda em **ordem inversa**", "estornar a cobrança antes de liberar o estoque deixa uma janela em que o cliente não tem dinheiro nem produto"], ["2", "uma compensação que falha **não é engolida**", "um estorno que não passa deixa o sistema inconsistente, e isso precisa chegar a um humano — `ok` continua `no` mesmo que o resto tenha desfeito"], ["3", "cada passo tem **chave de idempotência estável**", "derivada do id da saga mais o nome do passo: reprocessar uma fila não cobra duas vezes"]]}},
...
s := Malha.saga("checkout", $"pedido-{id}",
  ...
  {"h2": "Onde continuar"},
  {"cards": [{"href": "/docs/tecnicas/api", "title": "OpenAPI", "meta": "do código para o contrato", "desc": "OpenAPI, Insomnia, Postman e curl, gerados das rotas."}, {"href": "/docs/kiln/tempo-real", "title": "WebSocket e SSE", "desc": "Quando o servidor precisa falar primeiro."}, {"href": "/docs/tecnicas/observar", "title": "Observabilidade", "desc": "Métrica, traço e linhagem."}, {"href": "/docs/devops", "title": "DevOps", "desc": "Os artefatos que levam os serviços ao ar."}, {"href": "/docs/exercicios/32-microservicos", "title": "Os exercícios", "meta": "227 e 228", "desc": "Dois serviços de verdade sobre sockets, e uma saga que compensa."}]},
];

const headings = [{ id: 'uma-chamada', text: "Uma chamada", level: 2 as const }, { id: 'status-0-e-o-terceiro-estado', text: "`status: 0` é o terceiro estado", level: 3 as const }, { id: 'retry-so-o-que-e-seguro', text: "Retry: só o que é seguro", level: 2 as const }, { id: 'recuo-exponencial-com-tremor', text: "Recuo exponencial, com tremor", level: 3 as const }, { id: 'disjuntor', text: "Disjuntor", level: 2 as const }, { id: 'descoberta', text: "Descoberta", level: 2 as const }, { id: 'o-rastro-do-pedido', text: "O rastro do pedido", level: 2 as const }, { id: 'observar', text: "Observar", level: 2 as const }, { id: 'um-sistema-em-tres-servicos', text: "Um sistema em três serviços", level: 2 as const }, { id: 'saga-escritas-que-precisam-acontecer-juntas', text: "Saga: escritas que precisam acontecer juntas", level: 2 as const }, { id: 'tres-coisas-que-a-diferenciam-de-um-monitor-com-ensure', text: "Três coisas que a diferenciam de um `monitor` com `ensure`", level: 3 as const }, { id: 'conferir-roda-antes-de-executar', text: "`conferir()` roda antes de executar", level: 3 as const }, { id: 'o-diario-e-por-que-a-cada-passo', text: "O diário, e por que a cada passo", level: 3 as const }, { id: 'o-que-este-modulo-nao-e', text: "O que este módulo NÃO é", level: 2 as const }, { id: 'onde-continuar', text: "Onde continuar", level: 2 as const }];
