...
out $"{r['registros']} registros, cadeia fecha: {r['ok']}"`, title: `trilha encadeada` },
  ...
  {"h2": "Funções (51)"},
  {"table": {"head": ["Assinatura"], "rows": [["`analisar(fonte, caminho='')`"], ["`assinar(valor, chave, proposito='', quando=None)`"], ["`assinar_url(url, chave, prazo=3600, quando=None)`"], ["`auditoria(caminho)`"], ["`caminho_seguro(base, pedido)`"], ["`chave_de_assinatura(bytes_=32)`"], ["`codigos_de_recuperacao(quantos=10, grupos=3, tamanho=4)`"], ["`conferir_url(url, chave, quando=None)`"], ["`conferir_vazamento(senha, respostas)`"], ["`e_alta_entropia(texto, minimo=3.5)`"], ["`e_segredo(v)`"], ["`entropia(texto)`"], ["`escapar_atributo(texto)`"], ["`escapar_cabecalho(texto)`"], ["`escapar_csv(valor)`"], ["`escapar_html(texto)`"], ["`escapar_js(texto)`"], ["`escapar_log(texto)`"], ["`escapar_regex(texto)`"], ["`escapar_shell(texto)`"], ["`escapar_sql_like(texto, escape='\\\\')`"], ["`escapar_url(texto)`"], ["`exigir_politica(senha, opcoes=None)`"], ["`exigir_sem_segredo(texto, onde='')`"], ["`forca_da_senha(senha)`"], ["`host_privado(host)`"], ["`hotp(segredo, contador, digitos=6, algoritmo='sha1')`"], ["`json_seguro(texto, opcoes=None)`"], ["`ler_assinado(token, chave, prazo=None, proposito='')`"], ["`limitador(limite, periodo=60.0, rajada=None)`"], ["`limpar_html(texto, opcoes=None)`"], ["`mascarar(valor, visivel=4)`"], ["`mascarar_pii(texto, tipos=None)`"], ["`nome_de_arquivo_seguro(nome, padrao='arquivo')`"], ["`numero_seguro(texto, minimo=None, maximo=None, inteiro=True)`"], ["`padroes_de_segredo()`"], ["`politica(senha, opcoes=None)`"], ["`prefixo_vazamento(senha)`"], ["`procurar_segredos(texto, opcoes=None)`"], ["`redigir(texto)`"], ["`redirecionamento_seguro(destino, permitidos=None, padrao='/')`"], ["`regras_de_analise()`"], ["`segredo(valor, rotulo='segredo')`"], ["`sem_controle(texto, permitir_quebra=False)`"], ["`tentativas(limite=5, base=30.0, teto=3600.0)`"], ["`totp_agora(segredo, janela=30, digitos=6, algoritmo='sha1', quando=None)`"], ["`totp_conferir(segredo, codigo, janela=30, digitos=6, algoritmo='sha1', tolerancia=1, quando=None)`"], ["`totp_segredo(bytes_=20)`"], ["`totp_uri(segredo, conta, emissor='', digitos=6, janela=30, algoritmo='sha1')`"], ["`url_segura(url, opcoes=None)`"], ["`vazada(senha, tempo_limite=5.0)`"]]}},
];

const headings = [{ id: 'escapar-e-por-destino-nunca-em-geral', text: "Escapar é por destino, nunca “em geral”", level: 2 as const }, { id: 'o-segredo-que-se-recusa-a-aparecer', text: "O segredo que se recusa a aparecer", level: 2 as const }, { id: 'varredura-de-segredo-e-o-que-a-faz-calar', text: "Varredura de segredo, e o que a faz calar", level: 2 as const }, { id: 'entrada-hostil', text: "Entrada hostil", level: 2 as const }, { id: 'segundo-fator-e-o-token-com-prazo', text: "Segundo fator, e o token com prazo", level: 2 as const }, { id: 'senha-a-nota-nao-serve-a-lista-serve', text: "Senha: a nota não serve, a lista serve", level: 2 as const }, { id: 'auditoria-que-nao-pode-ser-editada-sem-deixar-marca', text: "Auditoria que não pode ser editada sem deixar marca", level: 2 as const }, { id: 'o-que-este-modulo-nao-faz', text: "O que este módulo não faz", level: 2 as const }, { id: 'funcoes-51', text: "Funções (51)", level: 2 as const }];
