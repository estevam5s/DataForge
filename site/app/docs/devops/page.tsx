...
// A fonte e 'site/scripts/conteudo/devops.py' — mude la e rode o gerador.
...
const blocos: Bloco[] = [
  ...
  {"cards": [{"href": "/docs/devops/docker", "title": "Docker e Compose", "desc": "A imagem multiestágio, o compose com banco — e as cinco decisões que cada artefato carrega."}, {"href": "/docs/devops/kubernetes", "title": "Kubernetes", "desc": "Deployment, Service, Ingress, ConfigMap e HPA — com sondas e limites, que é o que falta em quase todo manifesto."}, {"href": "/docs/devops/helm-terraform", "title": "Helm e Terraform", "desc": "O chart para variar por ambiente, e o esqueleto de infraestrutura como código."}, {"href": "/docs/devops/github-actions", "title": "GitHub Actions", "desc": "O pipeline na ordem que economiza tempo — e o erro de análise anotado na linha do PR."}, {"href": "/docs/devops/actions-na-linguagem", "title": "Um passo em DataForge", "desc": "Saídas, variáveis, resumo, anotações, máscara e grupos — com Arcane.GitHub, e sem injeção."}, {"href": "/docs/devops/webhooks", "title": "Webhooks do GitHub", "desc": "A assinatura HMAC sobre os bytes originais, a entrega que chega duas vezes, e o 401 que não processa nada."}, {"href": "/docs/devops/api-github", "title": "A API do GitHub", "desc": "Issue, comentário no PR, status do commit, release e workflow_dispatch — com paginação e o limite de taxa."}, {"href": "/docs/devops/release", "title": "Release por tag", "desc": "Uma tag v* testa de novo, confere a versão do manifesto, empacota e publica."}, {"href": "/docs/devops/repositorio", "title": "O repositório", "desc": "Dependabot, CODEOWNERS, modelos de PR e de issue, e a proteção do ramo principal."}, {"href": "/docs/devops/gitlab", "title": "GitLab CI", "desc": "O mesmo pipeline no GitLab, com o relatório JUnit na página do merge request."}, {"href": "/docs/devops/ambiente-de-dev", "title": "Devcontainer e pre-commit", "desc": "Devcontainer para Codespaces e VS Code, e os ganchos do pre-commit que rodam o CI antes do commit."}, {"href": "/docs/devops/vm", "title": "Numa VM (systemd)", "desc": "A unidade systemd endurecida, o nginx na frente com TLS — e o que o Kiln não faz sozinho."}]},
...
dataforge devops --seco       # mostra o que faria, sem escrever`, lang: 'bash' },
  ...
  {"table": {"head": ["Comando", "Gera"], "rows": [["`devops init`", "tudo o que faz sentido para este projeto (o padrão)"], ["`devops docker`", "`Dockerfile`, `.dockerignore`, `docker-compose.yml`"], ["`devops ci github`", "`.github/workflows/ci.yml`"], ["`devops k8s`", "`deployment`, `service`, `ingress`, `configmap`, `hpa`"], ["`devops helm`", "um chart com `values.yaml`"], ["`devops terraform`", "o esqueleto"], ["`devops nginx`", "proxy reverso com TLS, WebSocket e SSE"], ["`devops observar`", "Prometheus, Grafana e OpenTelemetry"], ["`devops sbom`", "o inventário, em CycloneDX"], ["`devops secrets`", "`.env.example` e o `.gitignore`"], ["`devops doctor`", "o que falta para este projeto subir"]]}},
  ...
  {"table": {"head": ["No artefato", "Sem ele"], "rows": [["`USER forge`", "um escape de container vira **root no host**"], ["o manifesto copiado antes do código", "um commit numa linha reinstala tudo — o build vai de 8 s para 2 min"], ["`.env` no `.dockerignore`", "o segredo fica na camada, e `docker history` o mostra — **mesmo apagado numa camada seguinte**"], ["`resources` + as duas sondas no Deployment", "um pod come o nó inteiro; e o Service manda tráfego antes da hora"], ["`depends_on: service_healthy`", "a aplicação falha na primeira consulta, de forma intermitente"]]}},
...
ignite loja on 8080 at "0.0.0.0"`, lang: 'df' },
  ...
  {"table": {"head": ["Arquivo", "Faz"], "rows": [["`dataforge/devops.py`", "produz **texto**"], ["`dataforge/devops_cli.py`", "escreve **arquivo**"]]}},
  ...
  {"h2": "Onde continuar"},
  {"cards": [{"href": "/docs/instalacao/docker", "title": "A imagem oficial", "desc": "Rodar DataForge em container, sem instalar nada."}, {"href": "/docs/tecnicas/observar", "title": "Observabilidade", "desc": "Métrica, traço e linhagem — o que o 'devops observar' instrumenta."}, {"href": "/docs/tecnicas/microservicos", "title": "Microserviços", "desc": "O que muda quando a chamada atravessa a rede."}, {"href": "/docs/seguranca", "title": "Segurança", "desc": "Cabeçalhos, CSRF, limite de taxa e o que não vai ao repositório."}]},
];

const headings = [{ id: 'o-ecossistema-pagina-por-pagina', text: "O ecossistema, página por página", level: 2 as const }, { id: 'os-onze-subcomandos', text: "Os onze subcomandos", level: 2 as const }, { id: 'os-artefatos-saem-do-que-o-projeto-adota', text: "Os artefatos saem do que o projeto ADOTA", level: 2 as const }, { id: 'o-que-o-dockerfile-carrega-e-por-que', text: "O que o Dockerfile carrega, e por quê", level: 2 as const }, { id: 'doctor-e-ele-funciona-num-projeto-que-nao-compila', text: "`doctor` — e ele funciona num projeto que não compila", level: 2 as const }, { id: 'segredo-versionado-a-conferencia-e-no-git', text: "Segredo versionado: a conferência é no git", level: 3 as const }, { id: '--host0000-e-obrigatorio-dentro-de-um-container', text: "`--host=0.0.0.0` é obrigatório dentro de um container", level: 2 as const }, { id: 'kubernetes-o-que-os-manifestos-trazem', text: "Kubernetes: o que os manifestos trazem", level: 2 as const }, { id: 'o-yaml-e-escrito-a-mao', text: "O YAML é escrito à mão", level: 2 as const }, { id: 'duas-metades-separadas-de-proposito', text: "Duas metades separadas de propósito", level: 2 as const }, { id: 'o-que-ele-nao-e', text: "O que ele não é", level: 2 as const }, { id: 'onde-continuar', text: "Onde continuar", level: 2 as const }];
