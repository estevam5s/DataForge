...
const blocos: Bloco[] = [
  ...
  {"p": "Isso não é uma promessa em prosa: `doc/superficie.json` guarda a lista de tudo o que é público — as 81 palavras reservadas, as 228 funções embutidas, os 37 módulos e cada símbolo deles, os apelidos, os 53 comandos da CLI e os 177 códigos de erro. Um teste falha quando algo some dessa lista."},
  {"table": {"head": ["", "Quer dizer", "Exemplo"], "rows": [["**1.x.y → 2.0.0**", "um programa válido pode parar de funcionar", "uma palavra reservada some; um módulo muda de nome"], ["**1.2.0 → 1.3.0**", "há capacidade nova, e o que existia continua", "um módulo novo; um parâmetro opcional novo"], ["**1.2.3 → 1.2.4**", "correção, sem interface nova", "um bug de arredondamento; uma mensagem melhor"]]}},
  ...
  {"list": ["**Bytecode** — é interpretador de árvore com **compilação para fechamentos** (1,5× a 1,8×, medido). Uma VM de bytecode escrita em Python tem teto de ~6,5%, porque continua sendo Python rodando o laço de despacho; o resto exigiria sair do Python. Para CPU pesada a resposta é `P.map_processos` (4,71× em 10 núcleos) ou a ponte para o `numpy`.", "**HTTP/2 e TLS no Kiln** — ele roda sobre o `http.server` do Python, e implementá-los é reescrever um servidor de produção. Em produção pública: nginx ou Caddy na frente. **WebSocket existe** (`Kiln.ws`, `Kiln.sala`), com o RFC 6455 falado à mão, e SSE também.", "**`Cluster<T>` e `Vault<K,V>`** — o limite de um parâmetro de tipo é cobrado; o tipo do **conteúdo** de uma coleção, não.", "**Sincronização automática** — `mutex`, `semaforo`, `barreira`, `contador` e canal bloqueante existem, mas nada é aplicado sozinho: o `check` **avisa** sobre escrita concorrente, e recusar proibiria o uso correto com mutex.", "**Um *shim* de versão no PATH** — versões lado a lado e pino por projeto existem e são cobrados; o que não há é um atalho que resolva a versão antes de o Python subir."]},
  ...
  {"table": {"head": ["Dizia", "O que é hoje", "Como conferir"], "rows": [["não há autocompletar sensível a contexto", "`p.` oferece os campos e métodos do record; `c.` inclui o que veio da mãe; `self.` traz o campo criado com `self.x := …`; `xs.`, `t.` e `v.` vêm das tabelas do **interpretador**; e quando não prova o tipo, ele **cala**", "`pytest tests/test_lsp.py -k ponto`"], ["não há ponto de parada", "o DAP tem `setBreakpoints`, parada condicional, contagem de passagens, logpoint, parada por thread e **vigia** — de escrita e de leitura", "`pytest tests/test_dap.py`"], ["sem mutex; a coordenação é por `channel`", "`C.mutex()`, `C.com_trava`, semáforo, barreira, contador atômico, canal bloqueante, STM e atores", "o exercício 257, e `Arcane.Concurrent`"], ["o `match` não avisa se um membro de enum ficou de fora", "avisa em cinco formas, inclusive no padrão **aninhado** — `[Cor.A, x]` sem o `Cor.B`", "`dataforge check` num `match` incompleto"], ["generics documentam, não restringem", "`<T extends X>` é cobrado nas **duas** metades: o `check` na chamada e a execução no valor — inclusive através de um `adopt`", "`dataforge check` em `maior<T extends Number>(\"x\", \"y\")`"]]}},
