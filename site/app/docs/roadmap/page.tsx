...
const blocos: Bloco[] = [
  ...
  {"h2": "1.1 — Confiança"},
  {"list": ["**Verificação de exaustividade** em `match` sobre enum: avisar quando um membro ficou de fora. É o item de melhor relação custo/benefício que sobrou.", "**Contrato de trait**: falhar na declaração quando o blueprint não implementa os métodos, em vez de só na chamada.", "**Generics** — `Cluster<T>`, `Vault<K,V>`, ações genéricas.", "**Menos unário no formatador**: hoje `yield -1` vira `yield - 1`, e por isso ninguém roda `dataforge fmt`."]},
  {"h2": "1.2 — Ferramental"},
  {"list": ["**LSP**: autocompletar, ir-para-definição, renomear, tipo ao passar o mouse. O analisador já produz diagnósticos com linha e coluna — falta o servidor.", "**Depurador**: breakpoints, passo a passo, inspeção de variáveis.", "**Cobertura de testes** no `dataforge test`."]},
  {"h2": "1.3 — Ecossistema"},
  ...
  {"h2": "2.0 — Runtime"},
  {"list": ["**IR e máquina virtual de bytecode** — hoje é interpretador de árvore, sem otimização.", "**Cache de compilação**.", "**Empacotamento** — gerar um executável com o runtime embutido."]},
