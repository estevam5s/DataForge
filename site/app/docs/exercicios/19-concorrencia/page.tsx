...
assert valor bigger 0, "algo foi contado"`, lang: 'df' },
  ...
  {"h3": "Como evitar"},
  {"p": "Há duas respostas, e a primeira é a melhor: **não compartilhe**."},
  { code: `channel resultados
thread:
    resultados.send(calcular())`, lang: 'df' },
  {"p": "Cada thread envia o que produziu; a thread principal recebe e agrega. Ninguém escreve na mesma variável, então não há o que proteger."},
  {"p": "Quando o compartilhamento é inevitável, `Arcane.Concurrent` tem `mutex`, `semaforo`, `barreira`, `contador` atômico e `trava_leitura_escrita` — esta linha já disse que a linguagem não tinha nenhum deles, e ela tem. O que ela **não** faz é aplicar qualquer um sozinho: a escolha é de quem escreve, e o `dataforge check` avisa (`escrita-concorrente`) quando um `thread`, um `parallel` ou uma `route` escreve num nome que vem de fora."},
