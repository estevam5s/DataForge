...
V.subir(porta := 8501)""", "lang": "df"},
 ...
 {"cards": [
   ...
   {"href": "/docs/vitrine/painel", "title": "Painel profissional", "meta": "indicador, grade, fragmento", "desc": "O que separa um relatório de um painel — e as 27 formas de desenhar."},
   {"href": "/docs/vitrine/layout", "title": "Layout", "meta": "colunas, abas, cartões, malha", "desc": "Por que a área é um objeto, e não um bloco de contexto."},
   ...
   {"href": "/docs/vitrine/graficos", "title": "Gráficos", "meta": "27 tipos, em SVG", "desc": "A forma curta e a construída, e o que os dados precisam parecer."},
   ...
   {"href": "/docs/vitrine/referencia", "title": "Referência", "meta": "213 símbolos", "desc": "Tudo o que sai de `adopt Arcane.Vitrine`, em uma tabela."}]},
]},
...
relay importar, limpar""", "lang": "df"},
 ...
 {"table": {"head": ["Decisão", "Por quê"], "rows": [
   ["`upsert_many` com a chave natural", "reprocessar o mesmo arquivo não duplica"],
   ["lote de 500, e não linha a linha", "uma transação por linha é uma sincronização de disco por linha; uma transação só prende a escrita até o fim"],
   ["linha ruim vai para `recusadas`", "não derruba a importação, e não desaparece"]]}},
...
action test_a_api_responde():
 ...
 {"table": {"head": ["Nível", "Pega"], "rows": [
   ["ETL", "a vírgula decimal, o espaço em branco, a região com caixa diferente"],
   ["consultas", "o `GROUP BY` errado, o filtro que não filtra, o `JOIN` que perde linha"],
   ["painel", "o componente que sumiu, o filtro que não chega à consulta, o erro que a página engoliu"]]}},
...
out V.metricas()["media_ms"]""", "lang": "df"},
 {"table": {"head": ["Sintoma", "Causa provável"], "rows": [
   ["a taxa de acerto do cache é baixa", "a chave muda a cada execução — um argumento que é um vault novo, ou o relógio"],
   ["uma consulta aparece com `SCAN`", "falta índice na coluna do `WHERE`"],
   ["a média em ms sobe com o número de sessões", "cada sessão refaz a agregação; suba a `validade` do cache"],
   ["o primeiro clique é lento e os outros não", "é o cache frio, e está funcionando"]]}},
...
V.subir(porta := 8501, host := "127.0.0.1")""", "lang": "df"},
 ...
 {"h2": "Onde continuar"},
 {"cards": [
   {"href": "/docs/tecnicas/banco-de-dados/crud", "title": "O CRUD completo", "desc": "Cinco sistemas com o mesmo esqueleto."},
   {"href": "/docs/tecnicas/banco-de-dados/relatorios", "title": "Relatórios e busca", "desc": "Agregação, FTS5 e o plano de consulta."},
   {"href": "/docs/vitrine/graficos", "title": "Gráficos", "desc": "27 tipos, em SVG escrito no servidor."},
   {"href": "/docs/tecnicas/cobertura", "title": "Cobertura", "desc": "O que os testes deste projeto não exercitam."}]},
]},
# ══════════════════════════════════════════════════════════════
{
...
"blocos": [
 ...
 {"callout": {"tipo": "dica", "titulo": "O exemplo roda",
              "texto": "`examples/vitrine_painel_financeiro.df` monta o painel inteiro desta página e confere o resultado com os próprios `assert`. Para ver no navegador: `dataforge run examples/vitrine_painel_financeiro.df -- --servir`."}},
...
given len(escolhidas) bigger 0:
 ...
 {"table": {"head": ["Tipo de coluna", "O que ele desenha"],
            "rows": [
   ["`texto`", "o valor como veio"],
   ["`moeda` · `numero` · `percentual` · `compacto`", "formatado em pt-BR, alinhado à direita"],
   ["`data`", "`2026-09-20` vira `20/09/2026`"],
   ["`barra` · `progresso`", "uma barra dentro da célula, com o valor ao lado"],
   ["`mini`", "uma série miúda — a coluna carrega um cluster"],
   ["`selo`", "uma etiqueta colorida; a cor sai do valor"],
   ["`link` · `imagem` · `logico`", "âncora, miniatura e ✓/—"]]}},
...
given V.botao("Salvar"):
 ...
 {"table": {"head": ["Para responder", "O gráfico"],
            "rows": [
   ["quanto, ao longo do tempo", "`grafico_linha`, `grafico_area`, `grafico_area_empilhada`"],
   ["quanto, por categoria", "`grafico_barras`, `grafico_barras_h`, `grafico_pareto`"],
   ["de que isto é feito", "`grafico_pizza`, `grafico_rosca`, `grafico_treemap`, `grafico_barras_100`"],
   ["duas grandezas de escalas diferentes", "`grafico_combo` com `eixo_direito`"],
   ["de onde veio a diferença", "`grafico_cascata`"],
   ["quanto sobra em cada etapa", "`grafico_funil`"],
   ["estamos perto da meta?", "`medidor`, `grafico_bala`"],
   ["quando acontece", "`mapa_de_calor`, `grafico_calendario`"],
   ["como se distribui", "`histograma`, `grafico_caixa`"],
   ["um está ligado ao outro?", "`grafico_dispersao_xy`, `grafico_bolhas`"],
   ["comparar em vários critérios", "`grafico_radar`"],
   ["para onde foi", "`grafico_sankey`, `grafico_rede`"],
   ["quando cada coisa acontece", "`grafico_gantt`"],
   ["o mercado", "`grafico_velas`"],
   ["onde", "`grafico_mapa`"],
   ["a tendência ao lado de um número", "`mini_grafico`"]]}},
...
V.vault(V.segredos_mascarados())""", "lang": "df"},
 ...
 {"h2": "Onde continuar"},
 {"cards": [
   {"href": "/docs/vitrine/graficos", "title": "Gráficos", "desc": "A forma curta, a construída, e por que SVG no servidor."},
   {"href": "/docs/vitrine/referencia", "title": "Referência", "meta": "213 símbolos", "desc": "Tudo o que sai de `adopt Arcane.Vitrine`."},
   {"href": "/docs/vitrine/producao", "title": "Produção", "desc": "Sessão entre processos, saúde e métricas."},
   {"href": "/docs/vitrine/testes", "title": "Testar sem navegador", "desc": "A sonda clica, digita e pergunta."}]},
]},
]
