# -*- coding: utf-8 -*-
"""A extensão do VS Code — tudo que ela faz.

Esta página existe porque a `homepage` da extensão publicada aponta para
`/docs/editor`, e a rota **não existia**: 404 no marketplace, no lugar
onde quem instala vai procurar o que a coisa faz.
"""

TITULO = "A extensão do editor"
DESCRICAO = ("Cores, erros enquanto se digita, autocompletar, depurador com "
             "breakpoints, Big-O acima de cada ação, 52 comandos e o painel "
...
dataforge editor remove     # desinstala de todos""", "lang": "bash"},
 ...
 {"table": {"head": ["O que ele acha", "Exemplo"], "rows": [
 ...
 {"p": "Os nomes do **próprio arquivo** vêm antes dos 1500 símbolos da stdlib — é o que se procura em nove de cada dez vezes."},
 ...
 {"table": {"head": ["No painel", "O que se vê"], "rows": [
...
adopt Arcane.Math.{sqrt, floor}    // dois nomes""", "lang": "df"},
 ...
 {"p": "Antes havia um botão de rodar, e mais nada. Os 52 comandos viviam na paleta — e **a paleta só serve a quem já sabe que o comando existe**."},
 ...
 {"h2": "Configuração"},
 {"table": {"head": ["Chave", "Padrão", "O que faz"], "rows": [
...
dataforge dap     # Debug Adapter Protocol, no stdio""", "lang": "bash"},
 ...
 {"h2": "Onde continuar"},
 {"cards": [
   {"href": "/docs/cli", "title": "A CLI", "desc": "Os comandos que a extensão chama, com todas as opções."},
   {"href": "/docs/tecnicas/lsp", "title": "O servidor de linguagem", "desc": "Como o autocompletar e o hover são servidos do typechecker."},
   {"href": "/docs/big-o", "title": "Complexidade", "desc": "O que a lente estima, e o que ela não consegue provar."},
   {"href": "/docs/tecnicas/analise-estatica", "title": "Análise estática", "desc": "O que o check acha antes de rodar — e o que o faz calar."}]},
]


#: O MESMO conteúdo em dois endereços.
#:
#: '/docs/editor' é onde a `homepage` da extensão publicada aponta — e a
#: rota não existia: 404 no marketplace, no lugar onde quem instala vai
#: procurar o que a coisa faz.
#:
#: '/docs/tecnicas/editor' é onde ela morava, com metade do conteúdo, e
#: para onde apontam seis links internos, a página de instalação, a de
#: download e o `doc=` de uma mensagem de erro do `Arcane.Color`.
#:
...
#: tinha LSP e 52 comandos. O site é estático (`output: 'export'`), então
...
PAGINAS = [
    {"href": caminho, "title": TITULO, "description": DESCRICAO,
     "blocos": BLOCOS}
    for caminho in ("/docs/editor", "/docs/tecnicas/editor")
]
