...
const provar: Aba[] = [
  {
    titulo: 'Servidor de linguagem',
    texto: 'O mesmo analisador que o check usa, servindo o editor: autocompletar sensível a contexto, hover com a assinatura, ir-para-definição e renomear com segurança.',
    terminal: 'dataforge lsp',
    linhas: [
      { t: 'action dividir(total: Number, pessoas: Integer) -> Number:', c: 'dim' },
      { t: '' },
      { t: '   ┌─ hover ────────────────────────────────', c: 'dim' },
      { t: '   │ action dividir(total: Number, …) -> Number', c: 'info' },
      { t: '   │', c: 'dim' },
      { t: '   │ Divide a conta entre as pessoas,', c: 'ok' },
      { t: '   │ arredondando para cima.', c: 'ok' },
      { t: '   └────────────────────────────────────────', c: 'dim' },
      { t: '' },
      { t: 'a documentação sai do comentário que já estava lá', c: 'aviso' },
    ],
  },
  {
    titulo: 'Depurador',
    texto: 'Para onde você mandar, mostra o que está valendo e anda de uma instrução por vez. Qualquer expressão é avaliada no quadro onde você parou.',
    terminal: 'dataforge debug',
    linhas: [
      { t: 'dataforge debug conta.df --parar=12', c: 'cmd' },
      { t: '' },
      { t: '● conta.df:12  Assignment', c: 'aviso' },
      { t: '    11       cycle item in carrinho:', c: 'dim' },
      { t: '    12 →         total += item["preco"]', c: 'info' },
      { t: '' },
      { t: '(df) vars', c: 'cmd' },
      { t: '  <cycle>', c: 'info' },
      { t: '    item     {nome: Martelo, preco: 89.9}', c: 'ok' },
      { t: '    total    0', c: 'ok' },
    ],
  },
  {
    titulo: 'Complexidade na CLI',
    texto: 'dataforge big-o lê a árvore e diz a classe de cada ação — tempo e espaço. O CI reprova o que passar do teto que você definir.',
    terminal: 'dataforge big-o',
    linhas: [
      { t: 'dataforge big-o src/', c: 'cmd' },
      { t: '' },
      { t: '  ● buscar          O(log n)    tempo   O(1) espaço', c: 'ok' },
      { t: '  ● ordenar_e_par   O(n log n)  tempo   O(n) espaço', c: 'info' },
      { t: '  ▲ cruzar          O(n²)       tempo   O(n) espaço', c: 'aviso' },
      { t: '  ■ fib             O(2ⁿ)       tempo   O(n) espaço', c: 'erro' },
      { t: '' },
      { t: '  2 ação(ões) acima de O(n log n)', c: 'aviso' },
    ],
  },
  {
